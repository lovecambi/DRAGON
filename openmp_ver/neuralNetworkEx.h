...
//                cout << "feed forward uses " << time_elapse << " sec" << endl;
//                timer = omp_get_wtime();
            ...
            if (print_info)
                cout << "train loss = " << trainLoss[t] / numTrainSample << ",\t test loss = " << testLoss[t] / numTestSample << endl;
        }
    ...
    vector<T> train(int epoch, int batchSize, const vector<vector<vector<T>>>& trainData, const vector<vector<T>>& trainTarget, bool print_info = true)
    {
        ...
        for (int t = 0; t < epoch; t++) {
            ...
            if (print_info)
                cout << "train loss = " << trainLoss[t] / numTrainSample << endl;
        }
        return trainLoss;
    }
